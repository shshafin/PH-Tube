const loadData = async () => {
  ...
  data.data.forEach((category) => {
    ...
    <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                    <li class="me-2">
                        <a onclick="handleCategory('${category.category_id}')" href="#" class="inline-block px-5 py-2  bg-gray-300 rounded-md "
...
// cards
const handleCategory = async (video) => {
  const res = await fetch(
  ...
  if (data.status === false) {
    error.innerHTML = `<div class="text-center mt-40  ">
  <div class="flex items-center justify-center">
  <img class="" src="images/Icon.png" alt="">
  </div>
  <h1 class="text-3xl font-bold">Oops!! Sorry, There is no <br> content here</h1>
  ...
  data.data?.forEach((card) => {
    ...
    items.innerHTML = ` 
<div class="mx-4 lg:mx-0 md:mx-0" >
<div class="relative">
    <img class="h-56 w-96 rounded-md" src="${card?.thumbnail}" alt=""/>
    <div class="absolute bottom-4 right-2">
    <p class="bg-gray-900 text-white rounded-sm w-24 text-center">${
      card?.others.posted_date
        ? `${Math.floor(card?.others.posted_date / 3600)} hr`
        : ""
    }</p>
    </div>
    </div>
